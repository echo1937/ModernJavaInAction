...
public class BestPriceFinder {
    ...
    public List<String> findPricesInUSD(String product) {
        ...
        for (Shop shop : shops) {
            ...
            CompletableFuture<Double> futureRate = CompletableFuture.supplyAsync(() -> ExchangeService.getRate(Money.EUR, Money.USD))
                    .completeOnTimeout(ExchangeService.DEFAULT_RATE, 1, TimeUnit.SECONDS); // Timeout management added in Java 9

            CompletableFuture<Double> futurePriceInUSD =
                    CompletableFuture.supplyAsync(() -> shop.getPrice(product))
                            .thenCombine(futureRate, (price, rate) -> price * rate)
                            .orTimeout(3, TimeUnit.SECONDS); // Timeout management added in Java 9
