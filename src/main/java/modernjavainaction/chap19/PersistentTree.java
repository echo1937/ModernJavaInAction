...
public class PersistentTree {
    ...
    public static Tree fupdate1(String k, int newval, Tree t) {

        if (t == null) return new Tree(k, newval, null, null);

        if (k.equals(t.key)) return new Tree(k, newval, t.left, t.right);

        if (k.compareTo(t.key) < 0) return new Tree(t.key, t.val, fupdate1(k, newval, t.left), t.right);

        return new Tree(t.key, t.val, t.left, fupdate1(k, newval, t.right));
    }
